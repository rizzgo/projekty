...
for i in range(len(seg_frames_off)):
    seg_frames_off[i] = np.multiply(seg_frames_off[i], hann_window)

for i in range(len(seg_frames_on)):
    seg_frames_on[i] = np.multiply(seg_frames_on[i], hann_window)
...
# sound
wavfile.write('../audio/sim_maskon_{tone}_window.wav', 16000, sim_ontone)
wavfile.write('../audio/sim_maskon_{sentence}_window.wav', 16000, sim_onsentence)
