...
**/
	tBTNodePtr *LeafPtr = RootPtr;
	while (*LeafPtr != NULL) {
		if ((*LeafPtr)->Cont == Content) {
			return;
		}
		else if ((*LeafPtr)->Cont > Content) {
			LeafPtr = &(*LeafPtr)->LPtr;
		}
		else {
			LeafPtr = &(*LeafPtr)->RPtr;
		}
	}
	*LeafPtr = malloc(sizeof(struct tBTNode));
	if (*LeafPtr == NULL) { 
		fprintf(stderr, "Neúspešná alokácia uzla");
		return;
	}
	(*LeafPtr)->Cont = Content;
	(*LeafPtr)->LPtr = NULL;
	(*LeafPtr)->RPtr = NULL;
